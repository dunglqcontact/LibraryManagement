 ...
 * @author Dung
 */
public class InsertNewBookController extends HttpServlet {

    private static final String ERROR = "insertBook.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String bookTitle = request.getParameter("txtInsertBookTitle");
            String priceNotCast = request.getParameter("txtInsertPrice");
            int price = 0;
            String quantityOfBookNotCast = request.getParameter("txtInsertQuantityOfBook");
            int quantityOfBook = 0;
            String author = request.getParameter("txtInsertAuthor");

            if (bookID.isEmpty() || bookTitle.length() < 2) {
                error.setBookIDError("Book ID must not lower than 2 character");
                check = false;
            }

            if (dao.checkBookIDExisted(bookID)) {
                error.setBookIDError("Book ID is existed");
                check = false;
            }
            ...
            if (check) {
                int currentQuantityOfBook = quantityOfBook;
                BookDTO book = new BookDTO(bookID, bookTitle, quantityOfBook, currentQuantityOfBook, price, author);
                dao.insertBook(book);
                url = SUCCESS;
            } else {
                request.setAttribute("INSERT_BOOK_ERROR", error);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
